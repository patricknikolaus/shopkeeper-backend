class GamesController < ApplicationController
  def index
    random = rand(1..1000)
    if params[:search] == ""
      response = HTTP.headers("Authorization" => "Bearer #{Rails.application.credentials.igdb_key[:access_token]}", "Client-ID" => "#{Rails.application.credentials.igdb_key[:client_id]}").post("https://api.igdb.com/v4/games", :body => "fields id, name, platforms, cover.url; where platforms = 6 & themes != [42] & cover != null & rating > 50; limit 24; offset #{random};")
    else
