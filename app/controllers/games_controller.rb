class GamesController < ApplicationController
  def index
    random = rand(500)
    if params[:search] == ""
      response = HTTP.headers("Authorization" => "Bearer #{Rails.application.credentials.igdb_key[:access_token]}", "Client-ID" => "#{Rails.application.credentials.igdb_key[:client_id]}").post("https://api.igdb.com/v4/games", :body => "fields id, rating, name, platforms, genres.name, cover.url; where platforms = 6 & themes != [42] & cover != null & rating > 70; limit 24; offset #{random};")
    else
